...
logf = codecs.open("logfile.txt", "a", "utf-8")
...
while(now != end):
	...
	if success:
		logf.write(":  file "+ now.isoformat()+ " finished.   " + str(count) + " links added.\n")
	elif count != 0:
		logf.write(":  file "+ now.isoformat()+ " finished.   " + str(count) + " links added.\n")
	else:
		logf.write(":  file "+ now.isoformat()+ " failed.   " + str(count) + " links added.  Can't load page.\n")
	now += day
